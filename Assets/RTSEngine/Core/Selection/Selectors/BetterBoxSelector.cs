...
public class BetterBoxSelector : Selector
{
    ...
    private void UpdateSelectionToBounds(Bounds selectionBounds)
    {
        ...
        {
            if (selectionBounds.Contains(Camera.main.WorldToViewportPoint(selectableObject.Behaviour.transform.position)) && SelectionContext.Priority <= selectableObject.Priority)
