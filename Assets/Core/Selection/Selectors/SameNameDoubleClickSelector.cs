...
namespace JHiga.RTSEngine.Selection
{
    public class SameNameDoubleClickSelector : Selector
    {
        ...
        public override void InputStop()
        {
            if (Applicable)
            {
                string name = SelectionContext.selection[0].Entity.MonoBehaviour.name;
                SelectionContext.Select(SelectionContext.onScreen.Where(s => s.Entity.MonoBehaviour.name == name));
            }
