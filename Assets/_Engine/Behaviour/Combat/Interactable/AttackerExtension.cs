...
namespace JHiga.RTSEngine.Combat
{
    public class AttackerExtension : BaseInteractableExtension<AttackerProperties>, IAttacker
    {
        ...
        public AttackerExtension(IExtendableInteractable extendable, AttackerProperties properties) : base(extendable, properties)
        {
            ...
            set
            {
                if (value != _target)
                {
                    ...
                    if (Extendable.MonoBehaviour.enabled && Extendable.MonoBehaviour.gameObject.activeSelf)
                        Extendable.MonoBehaviour.StartCoroutine(TransitionCombatAnimLayer(value == null ? 1 : 0, value == null ? 0 : 1, 3));
                }
            }
        }
