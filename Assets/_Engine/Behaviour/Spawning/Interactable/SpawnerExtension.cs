...
namespace JHiga.RTSEngine.Spawning
{
    public class SpawnerExtension : BaseInteractableExtension<SpawnerProperties>, ISpawner
    {
        ...
        public SpawnerExtension(IExtendableInteractable entity, SpawnerProperties properties) : base(entity, properties)
        {
        ...
        public void RequestNext()
        {
            ...
            InteractableRegistry.RequireEntityID(Extendable.EntityId);
        ...
        private void DoSpawn()
        {
            if (nextID < 0 || !next.HasValue || next.Value.time > LockStep.time - timeStamp)
                return;
            next.Value.toSpawn.Spawn(Extendable.MonoBehaviour.transform.position + Properties.doorPosition, nextID, Extendable.PlayerId);
            next = null;
            nextID = -1;
        }
